...
def mfjc(self):
	...
	if not os.path.isdir(directory):
		...
		error_popup('critical','File path error','The Out/Log Directory does not exist. Please check that you have specified the correct directory.')
		errors += 1
	...
	if csv_list:
		if not os.path.isfile(csv_list):
			...
			if not os.path.isfile(csv_list): 
				...
				error_popup('critical','.csv file error','The Out/Log List .csv file cannot be found. Please ensure it is spelled correctly and is located within the Out/Log Directory.')
				errors += 1
	...
	if not os.path.isfile(sdf2xyz2sdf_Directory):
		...
		error_popup('critical','File path error','The sdf2xyz2sdf directory specified is not valid. Please ensure that sdf2tinkerxyz.exe can be found within this directory.')
		errors += 1
	...
	for ttemp in ttemps:
		if int(ttemp) <= 0:
			...
			error_popup('warning','Teff error','Temperatures and/or grid spacing cannot be negative!')
			errors += 1
	...
	if len(ttemps) == 1:
		try:
			...
			self.ui.t1le_4.setStyleSheet('background-color: rgb(255, 0, 0)')
			error_popup('warning','Teff error','Tbath must be an integer.')
			errors += 1
	...
	# If more than 3 entries are given in the temp list, ensure that all entries are integers
		...
		try:
			...
			self.ui.t1le_4.setStyleSheet('background-color: rgb(255, 0, 0)')
			error_popup('warning','Teff error','Tbath, T_eff_max, and the temperature grids must be integers separated by a comma.')
			errors += 1

		# Check if the second temperature is larger than the first one
		if int(ttemps[1]) <= int(ttemps[0]):
			self.ui.t1le_4.setStyleSheet('background-color: rgb(255, 0, 0)')
			error_popup('warning','Teff error','T_eff_max (entry #2) must be larger than Tbath (entry #1).')
			errors += 1

	# If the temperature list is not 1 or 3 entries long, raise an error
	else:
		self.ui.t1le_4.setStyleSheet('background-color: rgb(255, 0, 0)')
		error_popup('warning','Teff error','The temp list must contain either 1 entry (T_bath), or three entries (T_bath, T_eff_max, and a grid size separated by a comma). Other array sizes are not permitted. Please see the .mfj creator section of the manual for more details.')
		errors += 1
	...
	if not (v_int / n_cores).is_integer() or not (b_int / n_cores).is_integer():
		...
		error_popup('warning','Error','Number of velocity grid points (inp) and impact parameters points (imp) must be divisible by the number of cores that the user intends on using.')
		errors += 1

	# Failsafe to ensure that erroneous parameters do not proceed to .mfj file creation 
	if errors != 0:
		return
