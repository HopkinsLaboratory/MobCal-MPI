...
C**** Calculate q1st terms here
 ...
 4011 continue
      ...
      do 4040 ic=1,itn                                    ! start loop over cycles
       if(imyrank.eq.0)then
...
c       endif
c
      do 4010 ig=1,inp                                    ! start loop over velocity points
        gst2=pgst(ig)*pgst(ig)
        v=dsqrt((gst2*eo)/(0.5d0*mu))
C        if(imyrank.eq.0)then
...
C        endif
        ...
        mpitemp2=0.0d0
        mpitemp3=0.0d0
        do 4000 im=1,imp_per_node                         ! start loop over MC points
         rnb=xrand()
         call rantate
         bst2=rnb*b2max(ig)
         b=ro*dsqrt(bst2)
...
4000    continue
c*AH                                                      ! collect MC data of all nodes
...
c
c       if(imyrank.eq.0)then
c         write(8,676) pgst(ig),mpitemp1,mpitemp2,mpitemp3                 ! this is still printed correctly
c       else
...
c
      if(im2.eq.0)then
       if(imyrank.eq.0)then
        ...
        write(8,675)
        do ig=1,inp
         write(8,676) pgst(ig),q1st(ig),q1STD(ig),                     ! these are now all zero for some reason
